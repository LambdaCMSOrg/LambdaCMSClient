...
function SidebarButton({label, Icon, to, onClick }) {
        ...
        <div className="flex justify-end items-center w-full h-[5em]">
            {label === "Logout" ? (
                <button
                    ...
                    <Icon className="w-5 h-5 text-[var(--color-textB)]" />
                    <span className="text-xl font-medium text-[var(--color-textB)]">{label}</span>
                </button>
            ) : (
                <Link
                    to={to || "#"}
