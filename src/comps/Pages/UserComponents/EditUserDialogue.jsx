...
export function EditUserDialogue({ onSubmit, onCancelEdit, setForm, form, error, isEditing }) {
    function validate(f) {
        if (!f.username.trim()) {
            ...
            <div className="mt-8 rounded-2xl border border-gray-200 bg-gray-50 p-4 shadow-sm">
                ...
                <form onSubmit={e => onSubmit(e, form)} className="flex flex-col gap-3 sm:flex-row sm:items-end">
                    ...
                    <label className="flex-1">
                        ...
                        <RoleSelector onChange={e => setForm(f => ({ ...f, role: e }))} defaultSelected={form.role}/>
                    </label>
                    <div className="flex gap-2">
                        ...
                        {isEditing && (
                            <button type="button" onClick={onCancelEdit} className="rounded-lg border border-gray-300 px-4 py-2 text-sm hover:bg-gray-50">
                                Cancel
                            </button>
                        )}
                    </div>
